...
| Módulo | Contenido |
| --- | --- |
| [`strategies.py`](strategies.py) | `rollback_isolation`, `rollback_scale_to_zero`, `rollback_increase_monitoring`: revierten el estado real (`FakeClusterState`/`FakeReplicaState`/`FakeMonitoringState`) que modificó el executor correspondiente; `mark_rolled_back` produce la copia del `ActionResult` original con `status="rolled_back"` y `rollback_ref` |
